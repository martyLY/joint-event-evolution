...
class Cosine:
    ...
    def similar0(self, source, target):
        a=document2(source)
        b=document2(target)
        tags = {}
        tagsa = {}

        if a.keywords not in tags:
            tags[a.keywords] = 0
        tags[a.keywords] += 1

        if b.keywords not in tagsa:
            tagsa[b.keywords] = 0
        tagsa[b.keywords] += 1

        numerator = sum([source[keywords] * target[keywords] for keywords in source if keywords in target])
        sourceLen = math.sqrt(sum([value * value for value in tags.values()]))
        targetLen = math.sqrt(sum([value * value for value in tagsa.values()]))
